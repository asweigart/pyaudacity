...
def do(command):  # type: (str) -> str
    ...
    _print(response)  # DEBUG
    if '\nBatchCommand finished: Failed!\n' in response:
        raise PyAudacityException(response[:response.find('\nBatchCommand finished: Failed!\n')])
...
def open(filename, add_to_history=False):  # type: (Union[str, Path], bool) -> str
...
def close():  # type: () -> str
    """Closes the current project window, prompting you to save your work
    if you have not saved."""
    _requireInteraction()
    return do('Close')

def page_setup():  # type: () -> str
    """Opens the standard Page Setup dialog box prior to printing."""
    _requireInteraction()
    return do('PageSetup')

def print():  # type: () -> str
    """Prints all the waveforms in the current project window (and the
    contents of Label Tracks or other tracks), with the Timeline above.
    Everything is printed to one page."""
    _requireInteraction()
    return do('Print')

def exit():  # type: () -> str
    """Closes all project windows and exits Audacity. If there are any unsaved
    changes to your project, Audacity will ask if you want to save them."""
    _requireInteraction()
    return do('Exit')

# The save() function uses the SaveProject2 macro, not the Save macro which only opens the Save dialog.
def save(filename, add_to_history=False, compress=False):  # type: (Union[str, Path], bool, bool) -> str
...
def save_as():  # type: () -> str
    """Same as save(), but allows you to save a copy of an open project
    to a different name or location."""
    _requireInteraction()
    return do('SaveAs')

def export_mp3():  # type: () -> str
    """Exports to an MP3 file."""
    _requireInteraction()  # If there's no audio, a dialog appears with "Warning: All audio is muted."
    return do('ExportMp3')

def export_wav():  # type: () -> str
    """Exports to an WAV file."""
    _requireInteraction()  # If there's no audio, a dialog appears with "Warning: All audio is muted."
    return do('ExportWav')

def export_ogg():  # type: () -> str
...
def export(filename, num_channels=1):  # type: (Union[str, Path], int) -> str
...
def import_audio(filename):  # type: (Union[str, Path]) -> str
...
def split_delete_labels():  # type: () -> str
...
def left_at_playback_position():  # type: () -> str
...
def right_at_playback_position():  # type: () -> str
...
def sel_track_start_to_cursor():  # type: () -> str
...
def next_higher_peak_frequency():  # type: () -> str
...
def next_lower_peak_frequency():  # type: () -> str
...
def sel_prev_clip_boundary_to_cursor():  # type: () -> str
...
def sel_cursor_to_next_clip_boundary():  # type: () -> str
...
def show_transport_t_b():  # type: () -> str
...
def show_spectral_selection_t_b():  # type: () -> str
...
def mix_and_render_to_new_track():  # type: () -> str
...
def unmute_all_tracks():  # type: () -> str
...
def chirp(start_freq=440, end_freq=1320, start_amp=0.8, end_amp=0.1, waveform='Sine', interpolation='Linear'):
    ...
    if waveform.lower() not in ('sine', 'square', 'sawtooth', 'square, no alias'):
        raise PyAudacityException('waveform argument must be one of "Sine", "Square", "Sawtooth", or "Square, no alias"')
    ...
    return do('Chirp: StartFreq="{}" EndFreq="{}" StartAmp="{}" EndAmp="{}" Waveform="{}" Interpolation="{}"'.format(start_freq, end_freq, start_amp, end_amp, waveform, interpolation))
...
def tone(frequency=440, amplitude=0.8, waveform='Sine', interpolation='Linear'): # type: (float, float, str, str) -> str
    ...
    return do('Tone: Frequency="{}" Amplitude="{}" Waveform="{}" Interpolation="{}"'.format(frequency, amplitude, waveform, interpolation))
...
def rhythm_track(tempo=0, time_signature=0, swing=0, bars=0, click_track_duration=0, offset=0, click_type='Metronome', high=0, low=0):
    ...
    if not isinstance(click_track_duration, (float, int)):
        raise PyAudacityException('click_track_duration argument must be float or int, not ' + str(type(click_track_duration)))
    if not isinstance(offset, (float, int)):
        raise PyAudacityException('offset argument must be float or int, not ' + str(type(offset)))
    if click_type.lower() not in ('metronome', 'ping (short)', 'ping (long)', 'cowbell', 'resonantnoise', 'noiseclick', 'drip (short)', 'drip (long)'):
    ...
    click_type = {'metronome': 'Metronome', 'ping (short)': 'Ping (short)', 'ping (long)': 'Ping (long)', 'cowbell': 'Cowbell', 'resonantnoise': 'ResonantNoise', 'noiseclick': 'NoiseClick', 'drip (short)': 'Drip (short)', 'drip (long)': 'Drip (long)'}[click_type.lower()]

    # The documentation has the parameters as lowercase, so I do too:
    return do('RhythmTrack: tempo="{}" timesig="{}" swing="{}" bars="{}" click-track-dur="{}" offset="{}" click-type="{}" high="{}" low="{}"'.format(tempo, time_signature, swing, bars, click_track_duration, offset, click_type, high, low))

def risset_drum():
    """Produces a realistic drum sound."""
    pass # TODO

def manage_effects():  # type: () -> str
    """Selecting this option from the Effect Menu (or the Generate Menu or Analyze Menu) takes you to a dialog where you can enable or disable particular Effects, Generators and Analyzers in Audacity. Even if you do not add any third-party plugins, you can use this to make the Effect menu shorter or longer as required. For details see Plugin Manager."""
    return do('ManageEffects')

def repeat_last_effect():  # type: () -> str
...
def spectral_edit_parametric_eq():
...
def manage_analyzers():  # type: () -> str
    """Selecting this option from the Effect Menu (or the Generate Menu or Analyze Menu) takes you to a dialog where you can enable or disable particular Effects, Generators and Analyzers in Audacity. Even if you do not add any third-party plugins, you can use this to make the Effect menu shorter or longer as required. For details see Plugin Manager."""
    return do('ManageAnalyzers')

# NOTE THE SPELLING OF "ANALYSER"
def contrast_analyser():  # type: () -> str
...
def apply_macros_palette():  # type: () -> str
...
def play_to_selection():  # type: () -> str
...
def play_before_selection_start():  # type: () -> str
...
def play_after_selection_start():  # type: () -> str
...
def play_before_selection_end():  # type: () -> str
...
def play_after_selection_end():  # type: () -> str
...
def play_before_and_after_selection_start():  # type: () -> str
...
def play_before_and_after_selection_end():  # type: () -> str
...
def play_cut_preview():  # type: () -> str
    """Plays audio excluding the selection."""
    return do('PlayCutPreview')

def select_tool():  # type: () -> str
    """Chooses Selection tool."""
    return do('SelectTool')

def envelope_tool():  # type: () -> str
    """Chooses Envelope tool."""
    return do('EnvelopeTool')

def draw_tool():  # type: () -> str
    """Chooses Draw tool."""
    return do('DrawTool')

def zoom_tool():  # type: () -> str
    """Chooses Zoom tool."""
    return do('ZoomTool')

def multi_tool():  # type: () -> str
    """Chooses the Multi-Tool."""
    return do('MultiTool')

def prev_tool():  # type: () -> str
...
def prev_track():  # type: () -> str
    """Focus one track up."""
    return do('PrevTrack')

def next_track():  # type: () -> str
    """Focus one track down."""
    return do('NextTrack')

def first_track():  # type: () -> str
    """Focus on first track."""
    return do('FirstTrack')

def last_track():  # type: () -> str
    """Focus on last track."""
    return do('LastTrack')

def shift_up():  # type: () -> str
    """Focus one track up and select it."""
    return do('ShiftUp')

def shift_down():  # type: () -> str
...
def toggle_alt():  # type: () -> str
    """Toggle focus on current track."""
    return do('ToggleAlt')

def cursor_left():  # type: () -> str
...
def cursor_short_jump_left():  # type: () -> str
...
def cursor_short_jump_right():  # type: () -> str
...
def cursor_long_jump_left():  # type: () -> str
...
def cursor_long_jump_right():  # type: () -> str
